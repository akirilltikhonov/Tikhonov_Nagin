...
%Dksi
UGOL1_rad = deg2rad((Options.Ufi1deg)*sin((1/Options.F_frame)*(2*pi/Options.Tturn)))
UGOL2_rad = deg2rad((Options.Ufi2deg)*sin((1/Options.F_frame)*(2*pi/Options.Tturn)))
UGOL3_rad = deg2rad((Options.Ufi3deg)*sin((1/Options.F_frame)*(2*pi/Options.Tturn)))
UGOL_rad = sqrt(UGOL1_rad^2+UGOL2_rad^2+UGOL3_rad^2)

ksi = 0.000001;     % Optimal order = 6 for angles~10 deg and Tturn=300--> min RMSE  
