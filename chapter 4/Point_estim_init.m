...
%Dx2
Point_estim.filter.x2 = [0; 0; 0; 1];  %[0.9; 0.435; 0; 0];  %[-9127; 0.2359; 0.2359; 0.2359];                   %a priori/starting state vector (quaternion camera orientation). 0 1 0 0 - camera rotate +180, 0, 0 deg relative X,Y,Z
...
%Dksi
Point_estim.filter.ksi1_x2_sko = 0.01 + 10*(deg2rad(Options.Ufi1deg))*sin((1/Options.F_frame)*(2*pi/Options.Tturn));
Point_estim.filter.ksi2_x2_sko = 0.01 + 10*(deg2rad(Options.Ufi2deg))*sin((1/Options.F_frame)*(2*pi/Options.Tturn));
Point_estim.filter.ksi3_x2_sko = 0.01 + 10*(deg2rad(Options.Ufi3deg))*sin((1/Options.F_frame)*(2*pi/Options.Tturn));
Point_estim.filter.ksi_x2_sko = 0.01 + sqrt((Point_estim.filter.ksi1_x2_sko)^2 + (Point_estim.filter.ksi2_x2_sko)^2 + (Point_estim.filter.ksi3_x2_sko)^2);
