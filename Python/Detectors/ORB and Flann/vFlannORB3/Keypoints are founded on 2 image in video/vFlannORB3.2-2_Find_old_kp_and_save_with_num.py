...
img1_1 = cv2.imread("query1.jpg", cv2.IMREAD_GRAYSCALE)   #queryImage1
img1_2 = cv2.imread("query2.jpg", cv2.IMREAD_GRAYSCALE)   #queryImage2
...
while True:
   ...
   if len(kp2) < 2:
      ...
      # Show result
      cv2.imshow("queryImage1", img3_1)
      cv2.imshow("queryImage2", img3_2)
   ...
   kp11 = []  # Create list for saving founded old keypoints in queryImage 1 and 2
   kp11_1 = []  # Create list for saving founded old keypoints in queryImage1
   kp11_2 = []  # Create list for saving founded old keypoints in queryImage1
   kp22 = []  # Create list for saving founded old keypoints in trainImage

   for mat in good_matches:
      kp11.append(kp1[mat.queryIdx])  # save founded old keypoints in queryImage 1 and 2
      if mat.queryIdx  < 100:
         kp11_1.append(kp1[mat.queryIdx])  # save founded old keypoints in queryImage1
      else:
         kp11_2.append(kp1[mat.queryIdx])  # save founded old keypoints in queryImage2
      kp22.append(kp2[mat.trainIdx])  # save founded old keypoints in trainImage

   # Draw keypoints
   img3_1 = cv2.drawKeypoints(img1_1, kp11_1, None, color=(0, 0, 255), flags=0)  # queryImage1
   img3_2 = cv2.drawKeypoints(img1_2, kp11_2, None, color=(0, 0, 255), flags=0)  # queryImage2
   ...
   for p in range(0, len(good_matches)):
      ...
      if good_matches[p].queryIdx < 100:
         cv2.putText(img3_1, '{}'.format(Num), (int(kp11[p].pt[0]), int(kp11[p].pt[1])), font, 0.5, (0, 0, 255), 1,
                     cv2.LINE_AA)
      else:
         cv2.putText(img3_2, '{}'.format(Num), (int(kp11[p].pt[0]), int(kp11[p].pt[1])), font, 0.5, (0, 0, 255), 1,
                     cv2.LINE_AA)
   ...
   num22 = []
   for p in range(0, len(good_matches)):
      num22.append(good_matches[p].queryIdx+1)
...
# Save for python
pickle.dump(temp_array, open("keypoints_numbers_database2.p", "wb"))
