...
# Load video
# cap = cv2.VideoCapture('source\_Nokia\VID1cal.mp4')
cap = cv2.VideoCapture('source\Mi5S\VID1.6.mp4')
...
font_size = 1         # Mi5S - 0.5; Nokia - 1
font_thickness = 3      # Mi5S - 1; Nokia - 3
...
while True:
   ...
   if FrameNumber == 1:
      ...
      for p in range(0, len(kp)):
         num.append(p + 1)
         cv2.putText(img1, '{}'.format(num[p]), (int(kp[p].pt[0]), int(kp[p].pt[1])), font, font_size, (0, 0, 255), font_thickness, cv2.LINE_AA)
      # Show result
   ...
   for p in range(0, len(kp)):
      cv2.putText(img, '{}'.format(num[p]), (int(kp[p].pt[0]), int(kp[p].pt[1])), font, font_size, (0, 0, 255), font_thickness, cv2.LINE_AA)
